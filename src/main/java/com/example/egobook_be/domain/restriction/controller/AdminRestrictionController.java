...
public class AdminRestrictionController implements AdminRestrictionControllerDocs {
    ...
    @Override
    public ResponseEntity<GlobalResponse<RestrictionCancelResDto>> cancelRestriction(Long restrictionId) {
        RestrictionCancelResDto response = adminRestrictionService.cancelRestriction(restrictionId);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(GlobalResponse.success(200, "사용자 제재 해제 성공", response));
    }
}
